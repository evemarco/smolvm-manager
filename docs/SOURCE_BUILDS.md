...
./scripts/build-smolvm.sh --version v1.14.6 --libkrun-mode auto

# Refuse to compile libkrun and libkrunfw
./scripts/build-smolvm.sh --version v1.14.6 --libkrun-mode bundled

# Force the patched submodules to compile locally
./scripts/build-smolvm.sh --version v1.14.6 --libkrun-mode source
```
...
- The musl agent build needs a musl cross toolchain. Debian ships `musl-tools`; Rocky/RHEL has no package. Build musl from source (`./configure --prefix=/opt/musl && make && make install`) and symlink the compiler the build expects: `ln -s /opt/musl/bin/musl-gcc /usr/local/bin/x86_64-linux-musl-gcc`.
- `bindgen` must find `libclang.so`, but upstream's helper probes `/usr/lib` only. On EL hosts the library lives in `/usr/lib64`; the build script now locates it across both prefixes and exports `LIBCLANG_PATH`. `clang-devel` must still be installed (see above).
